...
def checkHoldingRegisters(master, address, count, logger):
	try:	
		logger(master.execute(1,cst.READ_INPUT_REGISTERS,address,count))
		data = master.execute(1,cst.READ_INPUT_REGISTERS, address,count)
		return data
	except:
		return []
def checkForRegisters(master, address, count, logger):
	"""
	...
	#
	# @argument: master: what master are you polling?
...
def retrieveScaling(master,logger):
	"""
	retrieveScaling(): Retreives the scaling terms.
	
	@argument master: what master are you fetching it from?
	...
	"""
	data = checkForRegisters(master, 0x0000, 4, logger)
	print(data)
	return data

def retrieveADCVoltages(master, V_PU):
	"""
	# retreiveADCVoltages(): Retrives the filtered ADC voltages.
	# @argument master: what master are you asking?
	...
	"""
	data = checkForRegisters(master, 0x0018, 4, 100)
	if data == []: return []
	for i in range(4):
		data[i] *= (V_PU * (2 ** -15))
	return data

def retrieveADCCurrents(master, I_PU):
	"""
	...
	"""
	data = checkForRegisters(master, 0x001C, 2, 100)
	for i in range(2):
		data[i] *= I_PU
		data[i] *= (2**-15)
	return data
def retrieveOtherVoltages(master):
	""" 
	#retrieveOtherVoltages(): retrives miscellaneous voltages.
	# @argumetn master: what master are you asking?
	...
	"""
	data = checkForRegisters(master, 0x001E, 5, 100)
	data[0] *= 18.612 * (2 ** -15)
	data[1] *= 6.6 * (2 ** -15) 
	data[2] *= 18.612 * (2 ** -15)
	data[3] *= 3 * (2 ** -15)
...
def retrieveTemperatures(master):
	"""
	...
	"""
	data = checkForRegisters(master, 0x0023, 3,100)
	return data

def retrieveSlowADC(master, V_PU, I_PU):
	"""
	...
	"""
	data = checkForRegisters(master, 0x0026, 2, 100)
	data[0] *= V_PU * (2 ** -15)
	data[1] *= I_PU * (2 ** -15)
	return data

def retrieveMinMaxBatteryVoltages(master, V_PU):
	"""
...
def retrieveControllerFaults(master):
	...
	"""
	data = checkForRegisters(master,0x002C, 1, 1000)
	bitfield = data[0]
...
def retrieveChargeStatistics(master, V_PU):
	"""
	...
	"""
	data = checkForRegisters(master, 0x0033, 7, 100)
	if data == []: return []
	else:
...
def retrieveMPPTStatistics(master, V_PU,I_PU):
	"""
	...
	"""
	data = checkForRegisters(master, 0x003A, 5, 100)
	if data == []: return []
	else:
...
def retrieveTodaysValues(master,V_PU, I_PU):
	"""
	...
	"""
	data = checkForRegisters(master,0x0040, 16,10000)
	if data == []: return []
	else:
...
def checkChargeSettings(master, V_PU, I_PU):
	"""
	# Check EEPROM values.
	# checkChargeSettings(): returns an array full of charge settings. 
	# @argument master: what master are you reading?
	...
	"""
	#data = checkForRegisters(master, 0xE000, 32, 10000)
	data1 = checkHoldingRegisters(master,0xE000,4,100)
	data2 = checkHoldingRegisters(master,0xE004,4,100)
	data3 = checkHoldingRegisters(master,0xE008,4,100)
	print(data1,data2,data3)
	data = checkHoldingRegisters(master,0xE000,32,10000)
	if data == []: return []
